...
import pandas as pd
import json, hashlib, base64, re, os, re
...
def write_csv(p, df):
    ...
    if table:
        ...
        # Also update the local seed CSV for backup/local consistency.
        # This CSV should not be treated as the main production database.
        try:
...
def read_json(p):
    ...
    if table:
        _ensure_db_table_from_file(p)
        try:
            df = pd.read_sql_query(text(f'SELECT * FROM "{table}"'), get_engine()).fillna("")
...
def asset_img_url(relative_path):
    path = BASE / str(relative_path)
    if not path.exists():
        return ""
    return "data:image/jpeg;base64," + b64(path)
...
@st.cache_data
def universities(): return pd.read_csv(UNIS, keep_default_na=False).fillna("")
@st.cache_data
def criteria(): return pd.read_csv(CRITERIA, keep_default_na=False).fillna("")
@st.cache_data
def scholarship_rules(): return pd.read_csv(SCHOLARSHIPS, keep_default_na=False).fillna("") if SCHOLARSHIPS.exists() else pd.DataFrame()
